// Copyright (c) 2011-2020 <>< Charles Lohr - Under the MIT/x11 or
// NewBSD License you choose.
...
void AccCheck()
{
    ...
    do
    {
        ...
        accy = evt.vector.v[1];
        accz = evt.vector.v[2];
        mountainangle /*degrees*/ -=
            accz;  // * 3.1415 / 360.0;// / 100.0;
...
// writes the text to a file to path (example):
// /storage/emulated/0/Android/data/org.yourorg.cnfgtest/files
//  You would not normally want to do this, but it's an example of
//  how to do local storage.
...
void DrawHeightmap()
{
    ...
    mountainoffsety =
        mountainoffsety - ((mountainoffsety - 100) * .1);

    float eye[3] = {(float)(sin(mountainangle * (3.14159 / 180.0)) *
                            30 * sin(mountainoffsety / 100.)),
                    (float)(cos(mountainangle * (3.14159 / 180.0)) *
                            30 * sin(mountainoffsety / 100.)),
...
/*
void AudioCallback( struct CNFADriver * sd, short * out, short * in,
int framesp, int framesr )
{
        memset(out, 0, framesp*sizeof(uint16_t));
        if(suspended) return;
        if(!buttonstate[1]) return; // play audio only if ~touching
with two fingers audio_frequency = 440; for(uint32_t i = 0; i <
framesp; i++) { int16_t sample = INT16_MAX *
sin(audio_frequency*(2*M_PI)*(stream_offset+i)/SAMPLE_RATE); out[i]
= sample;
        }
...
void MakeNotification(const char* channelID,
                      ...
                      const char* message)
{
    ...
    jclass activityClass =
        env->GetObjectClass(ENVCALL gapp->activity->clazz);
    jmethodID MethodGetSystemService =
        env->GetMethodID(ENVCALL activityClass,
                         "getSystemService",
                         "(Ljava/lang/String;)Ljava/lang/Object;");
    jstring notificationServiceName =
        env->NewStringUTF(ENVCALL "notification");
    ...
    jmethodID notificationChannelConstructorID = env->GetMethodID(
        ENVCALL notificationChannelClass,
        "<init>",
        "(Ljava/lang/String;Ljava/lang/CharSequence;I)V");
    jobject notificationChannelObj =
        env->NewObject(ENVCALL notificationChannelClass,
                       notificationChannelConstructorID,
                       channelIDStr,
                       channelNameStr,
                       3);  // IMPORTANCE_DEFAULT
    jmethodID createNotificationChannelID =
        env->GetMethodID(ENVCALL NotificationManagerClass,
                         "createNotificationChannel",
                         "(Landroid/app/NotificationChannel;)V");
    env->CallVoidMethod(ENVCALL notificationServiceObj,
                        createNotificationChannelID,
                        notificationChannelObj);
    ...
    jmethodID eventConstructor = env->GetMethodID(
        ENVCALL classBuilder,
        "<init>",
        "(Landroid/content/Context;Ljava/lang/String;)V");
    jobject   eventObj = env->NewObject(ENVCALL classBuilder,
                                      eventConstructor,
                                      gapp->activity->clazz,
                                      channelIDStr);
    jmethodID setContentTitleID =
        env->GetMethodID(ENVCALL classBuilder,
                         "setContentTitle",
                         "(Ljava/lang/CharSequence;)Landroid/app/"
                         "Notification$Builder;");
    jmethodID setContentTextID =
        env->GetMethodID(ENVCALL classBuilder,
                         "setContentText",
                         "(Ljava/lang/CharSequence;)Landroid/app/"
                         "Notification$Builder;");
    ...
    // You could do things like setPriority, or setContentIntent if
    // you want it to do something when you click it.

    env->CallObjectMethod(
        ENVCALL eventObj, setContentTitleID, titleStr);
    env->CallObjectMethod(
        ENVCALL eventObj, setContentTextID, messageStr);
    env->CallObjectMethod(ENVCALL eventObj,
                          setSmallIconID,
    ...
    jobject notification =
        env->CallObjectMethod(ENVCALL eventObj, buildID);
    ...
    env->CallVoidMethod(
        ENVCALL notificationServiceObj, notifyID, id, notification);
...
{
    static int                   runno = 0;
    WebViewNativeActivityObject* wvn =
        (WebViewNativeActivityObject*)v;
    ...
    {
        // Invoke JavaScript, which calls a function to send a
        // webmessage back into C land.
...
void* JavscriptThread(void* v)
{
    ...
    jclass LooperClass =
        env->FindClass(envptr, "android/os/Looper");
    jmethodID myLooperMethod = env->GetStaticMethodID(
        envptr, LooperClass, "myLooper", "()Landroid/os/Looper;");
    jobject thisLooper = env->CallStaticObjectMethod(
        envptr, LooperClass, myLooperMethod);
    if(!thisLooper)
    {
        jmethodID prepareMethod = env->GetStaticMethodID(
            envptr, LooperClass, "prepare", "()V");
        env->CallStaticVoidMethod(
            envptr, LooperClass, prepareMethod);
        thisLooper = env->CallStaticObjectMethod(
            envptr, LooperClass, myLooperMethod);
        g_attachLooper = env->NewGlobalRef(envptr, thisLooper);
    }

    jmethodID getQueueMethod =
        env->GetMethodID(envptr,
                         LooperClass,
                         "getQueue",
                         "()Landroid/os/MessageQueue;");
    jobject lque = env->CallObjectMethod(
        envptr, g_attachLooper, getQueueMethod);

    jclass MessageQueueClass =
        env->FindClass(envptr, "android/os/MessageQueue");
    ...
    jclass MessageClass =
        env->FindClass(envptr, "android/os/Message");
    jfieldID objid = env->GetFieldID(
        envptr, MessageClass, "obj", "Ljava/lang/Object;");
    jclass PairClass = env->FindClass(envptr, "android/util/Pair");
    jfieldID pairfirst = env->GetFieldID(
        envptr, PairClass, "first", "Ljava/lang/Object;");

    while(1)
    {
        // Instead of using Looper::loop(), we just call next on the
        // looper object.
        jobject msg =
            env->CallObjectMethod(envptr, lque, nextMethod);
        ...
        // Check Object Type
        {
            ...
            jmethodID mid = env->GetMethodID(envptr,
                                             innerClass,
                                             "getClass",
                                             "()Ljava/lang/Class;");
            jobject   clsObj =
                env->CallObjectMethod(envptr, innerObj, mid);
            jclass clazzz = env->GetObjectClass(envptr, clsObj);
            mid           = env->GetMethodID(
                envptr, clazzz, "getName", "()Ljava/lang/String;");
            strObj =
                (jstring)env->CallObjectMethod(envptr, clsObj, mid);
        ...
        {
            // Special, Some Androids (notably Meta Quest) use a
            // different private message type.
            jfieldID mstrf =
                env->GetFieldID(envptr, innerClass, "a", "[B");
            jbyteArray jba = (jstring)env->GetObjectField(
                envptr, innerObj, mstrf);
            int      len    = env->GetArrayLength(envptr, jba);
            jboolean isCopy = 0;
            jbyte*   bufferPtr =
                env->GetByteArrayElements(envptr, jba, &isCopy);
            ...
            if(len >= 6)
            {
                ...
                tcpy[len - 6] = 0;
                snprintf(fromJSBuffer,
                         sizeof(fromJSBuffer) - 1,
                         "WebMessage: %s\n",
                         tcpy);
        ...
        else
        {
            jobject MessagePayload =
                env->GetObjectField(envptr, innerObj, pairfirst);
            // MessagePayload is a
            // org.chromium.content_public.browser.MessagePayload

            jclass mpclass =
                env->GetObjectClass(envptr, MessagePayload);

            // Get field "b" which is the web message payload.
            // If you are using binary sockets, it will be in `c`
            // and be a byte array.
            jfieldID mstrf = env->GetFieldID(
                envptr, mpclass, "b", "Ljava/lang/String;");
            jstring strObjDescr = (jstring)env->GetObjectField(
                envptr, MessagePayload, mstrf);

            const char* descr =
                env->GetStringUTFChars(envptr, strObjDescr, 0);
            snprintf(fromJSBuffer,
                     sizeof(fromJSBuffer) - 1,
                     "WebMessage: %s\n",
                     descr);
...
int main(int argc, char** argv)
{
    ...
    AAsset* file = AAssetManager_open(gapp->activity->assetManager,
                                      "asset.txt",
                                      AASSET_MODE_BUFFER);
    if(file)
    {
    ...
    // Disabled, for now.
    // InitCNFAAndroid( AudioCallback, "A Name", SAMPLE_RATE, 0, 1,
    // 0, SAMPLE_COUNT, 0, 0, 0 );
    ...
    while(1)
    {
        ...
        RunCallbackOnUIThread(
            (void (*)(void*))WebViewRequestRenderToCanvas,
            &MyWebView);
        ...
        for(i = 0; i < 400; i++)
        {
            RDPoint pp[3];
            pp[0].x =
                (short)(50 * sin((float)(i + iframeno) * .01) +
                        (i % 20) * 30);
            pp[0].y =
                (short)(50 * cos((float)(i + iframeno) * .01) +
                        (i / 20) * 20) +
                700;
            pp[1].x =
                (short)(20 * sin((float)(i + iframeno) * .01) +
                        (i % 20) * 30);
            pp[1].y =
                (short)(50 * cos((float)(i + iframeno) * .01) +
                        (i / 20) * 20) +
                700;
            pp[2].x =
                (short)(10 * sin((float)(i + iframeno) * .01) +
                        (i % 20) * 30);
            pp[2].y =
                (short)(30 * cos((float)(i + iframeno) * .01) +
                        (i / 20) * 20) +
                700;
            CNFGTackPoly(pp, 3);
        }
        ...
        // CNFGUpdateScreenWithBitmap does not have an implied
        // framebuffer swap.
        CNFGSwapBuffers();
