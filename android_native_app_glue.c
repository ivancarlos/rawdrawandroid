...
#endif
        // if( debug_capture_hook_function )
        // debug_capture_hook_function( readSize, buf );
    }
    return 0;
}
...
{
    // For additional debugging this can be enabled, but for now -
    ...
    /*
        ...
        LOGV("Config: mcc=%d mnc=%d lang=%c%c cnt=%c%c orien=%d
       touch=%d dens=%d " "keys=%d nav=%d keysHid=%d navHid=%d
       sdk=%d size=%d long=%d " "modetype=%d modenight=%d",
                   ...
                   size_t           savedStateSize)
{
    ...
    dup2(pfd[1], 1);
    dup2(pfd[1], 2);
    pthread_create(&debug_capture_thread,
                   &attr,
                   debug_capture_thread_fn,
                   android_app);
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // Handle calling events on the UI thread.  You can get
    // callbacks with RunCallbackOnUIThread.
    int msgpipemain[2];
    if(pipe(msgpipemain))
    {
        LOGE("could not create pipe: %s", strerror(errno));
    ...
    ALooper_addFd(looper,
                  android_app->uimsgread,
                  LOOPER_ID_MAIN_THREAD,
                  ALOOPER_EVENT_INPUT,
                  process_ui,
                  gapp);  // NOTE: Cannot use NULL callback
...
static void android_app_write_cmd(struct android_app* android_app,
                                  int8_t              cmd)
{
    if(write(android_app->msgwrite, &cmd, sizeof(cmd)) !=
       sizeof(cmd))
    {
...
static void onStart(ANativeActivity* activity)
{
    ...
    android_app_set_activity_state(
        (struct android_app*)activity->instance, APP_CMD_START);
}
...
static void onResume(ANativeActivity* activity)
{
    LOGV("Resume: %p\n", activity);
    android_app_set_activity_state(
        (struct android_app*)activity->instance, APP_CMD_RESUME);
}
...
static void onPause(ANativeActivity* activity)
{
    LOGV("Pause: %p\n", activity);
    android_app_set_activity_state(
        (struct android_app*)activity->instance, APP_CMD_PAUSE);
}
...
static void onStop(ANativeActivity* activity)
{
    LOGV("Stop: %p\n", activity);
    android_app_set_activity_state(
        (struct android_app*)activity->instance, APP_CMD_STOP);
}
                                  ...
                                  ANativeWindow*   window)
{
    ...
    android_app_set_window((struct android_app*)activity->instance,
                           window);
}
                                ...
                                AInputQueue*     queue)
{
    ...
    android_app_set_input((struct android_app*)activity->instance,
                          queue);
}
